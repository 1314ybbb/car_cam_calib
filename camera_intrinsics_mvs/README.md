...
先分别给两台相机完成内参标定。为相机 1、相机 2 各选一个单独图片文件夹。勾选“外参配对采集”并设置组号 1，将棋盘固定不动，分别从两台相机保存一张完整棋盘照片；同组号写入两张照片各自的 JSON。**两台相机拍完同一组之前，不得移动棋盘。**随后点击“下一组”，移动棋盘并停稳，再各拍一张。组与组之间棋盘可以移动，不需要始终处于同一世界位置。建议 10～20 组，包含不同距离和左右、上下倾角。可以开两个上位机窗口各连接一台相机；若逐台切换连接，拍同一组时棋盘必须始终保持不动。每张照片都需要检出完整 9×6 内角点。手持或运动中的棋盘应改为双相机硬件同步触发；仅给照片填写相同组号无法保证物理位姿相同。
