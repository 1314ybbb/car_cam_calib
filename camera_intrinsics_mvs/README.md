...
点击“选择内参 JSON”可切换到该相机已有内参；程序核对型号、序列号、图像尺寸及 9×6 棋盘规格。打开“实时位姿”后，棋盘完整入镜时显示角点与坐标轴，并报告棋盘中心在相机坐标系的 X/Y/Z、光心到棋盘中心的距离和重投影 RMS。关闭开关可继续普通预览。标定和位姿的毫米值都按输入的**实测格距**计算；曝光、增益不会改变几何内参，但对焦、镜头、分辨率、ROI 或装配改变后需重新验证。图形程序使用 `PyQt5`，若新机器缺少该包，需安装系统包 `python3-pyqt5`。
