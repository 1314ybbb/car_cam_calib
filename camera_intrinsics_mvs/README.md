...
增益也可在滑条右侧的“输入”框直接填写；最右侧显示相机实际回读值。连接相机后，上位机会自动查找匹配内参。点“查找已有内参”可以看到带型号、序列号、分辨率和 RMS 的候选列表。当前 `DB2189859` 的推荐内参为 `/home/ybbb/桌面/相机内参标定/DB2189859/calibration_recommended_20260917/intrinsics.json`；要手动选文件，请选名为 **`intrinsics.json`** 的文件，而不是每张照片旁的 `frame_XXXX.json`。

## 双相机棋盘格外参

先分别给两台相机完成内参标定。为相机 1、相机 2 各选一个单独图片文件夹。勾选“外参配对采集”并设置组号 1，将棋盘固定不动，分别从两台相机保存一张完整棋盘照片；同组号写入两张照片各自的 JSON。随后点击“下一组”，移动棋盘并停稳，再各拍一张。建议 10～20 组，包含不同距离和左右、上下倾角。可以开两个上位机窗口各连接一台相机；若逐台切换连接，拍同一组时棋盘必须始终保持不动。每张照片都需要检出完整 9×6 内角点。

点击“双相机棋盘格外参…”，填入两台相机的文件夹和各自的 `intrinsics.json`。可在连接相机时点“填入当前相机”。输入**实测**方格边长、预期夹角后计算；至少 5 组有效配对。算法按组号配对，固定两台相机内参，用 OpenCV `stereoCalibrate(CALIB_FIX_INTRINSIC)` 求相机 1 到相机 2 的旋转和平移，并检查每组位姿；不会把两台相机独立拍摄的内参照片自动配对。结果保存在相机 1 图片目录的 `stereo_时间戳/extrinsics.json`，旁边的 `detected_corners/` 用于核对角点。两台相机可使用不同分辨率，但各自照片须与各自内参匹配。相机装配、对焦或棋盘实测尺寸错误会影响外参质量。
