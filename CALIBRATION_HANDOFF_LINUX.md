...
整理日期：2026-09-17。本文为多传感器方案讨论的历史交接记录，不是逐字聊天导出。**双相机已在 2026-09-18 完成实验室内参与初步外参采集；当前硬件、IP、照片、参数和复算命令以 [双相机交接手册](HANDOFF_TWO_CAMERAS.md) 为准。**本文后文的相机进度描述保留当时语境，不能覆盖新结果。内容区分用户确认事实、讨论建议和未验证事项；不得把建议当成已完成实验。
