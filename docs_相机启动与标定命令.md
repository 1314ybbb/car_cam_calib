...
在窗口选择相机并连接。每台相机分别选择一个图片文件夹，按空格或点击“保存图片 + JSON”取帧；同名 JSON 保存相机身份和采集参数。填入**实际测量**的单格毫米数，拍摄至少 15 张有效棋盘照片（建议 25～35 张），点击“一键生成内参”。结果保存在该图片目录下的 `calibration_时间戳/intrinsics.json`，完成后自动选择。也可点击“查找已有内参”或“浏览内参文件…”载入旧结果，打开“实时位姿”开关查看棋盘坐标轴、X/Y/Z、光心距离和重投影误差。必须使用当前相机、当前分辨率对应的内参。

增益可拖动滑条，也可在旁边的“输入”框直接填写并按回车或点“应用增益”。“相机实际”显示四位小数的 SDK 回读值，可能与请求值略有差异。要选内参时，点“查找已有内参”；列表会标明相机序列号、分辨率和 RMS。当前相机 `DB2189859` 的推荐文件是：

```text
/home/ybbb/桌面/相机内参标定/DB2189859/calibration_recommended_20260917/intrinsics.json
```

照片旁的 `frame_XXXX.json` 是拍摄元数据，不是内参。

双相机外参：给两台相机分别选图片目录、分别载入内参。勾选“外参配对采集”，照片自动写入所选目录的 `stereo_pairs/` 子目录；原有内参照片不会被覆盖或混入，旧版根目录中带 `pair_id` 的照片也会被内参算法跳过。设置组号 1；**保持棋盘固定，直到两台相机各保存一张同组号照片**。两张拍完才点击“下一组”并移动棋盘，按此方式拍 10～20 组。组与组之间棋盘可以移动。打开“双相机棋盘格外参…”，填入两套 `stereo_pairs/` 目录和 `intrinsics.json`（“填入当前相机”会优先选择子目录），输入实测方格边长，点击计算。至少需要 5 组完整 9×6 棋盘配对；通过一致性检查后，结果在相机 1 图片目录中的 `stereo_时间戳/extrinsics.json`。未通过时只生成 `pair_review.json` 和角点图，按组号检查并重拍。两台相机可使用不同分辨率，但各自照片须与各自内参匹配。可以开两个上位机窗口分别连接，或在棋盘固定时切换相机。手持或运动中的棋盘需要同步触发两台相机；相同组号和事后的几何检查无法证明棋盘真的没有移动。

双相机自动配对采集：两台相机都位于电脑可通信的网段时，连接第一台，在上位机勾选“外参配对采集”；程序会自动打开另一台并左右显示。双画面下两台相机临时限制到 6 fps，并设置约 50 µs 包间隔，画面下方有两路丢包计数。按空格一次保存同组两张照片及 JSON：第一台在所选目录的 `stereo_pairs/`，第二台在 `stereo_pairs_第二台序列号/`。外参窗口中两个“填入当前相机”按钮可对应填入两套照片目录。关闭配对开关后，界面恢复单画面，相机原帧率和包间隔也会恢复。两路帧由软件读取，不能保证严格同步曝光；每组拍摄期间棋盘必须保持不动。

本机两台相机当前为 `DB2189859 → 192.168.1.213`、`DB2189878 → 192.168.1.214`，电脑网卡为 `192.168.1.88/24`。第二台的 `.214` 是通过 MVS Force IP 临时设置；若相机断电后又变成 `169.254.*`，在 MVS 的 **Tool → IP Configurator** 中给第二台设置独立的同网段地址。

外参采用 `p2 = R·p1 + T`，其中 `p1`、`p2` 分别为两台相机坐标系中的三维点；结果 JSON 同时给出相机 2 光心在相机 1 坐标系中的位置。预期的 60° 仅用于检查求出的偏航角，不参与强制拟合。
