...
双相机外参：给两台相机分别选图片目录、分别载入内参。勾选“外参配对采集”，设置组号 1；棋盘固定不动时，两台相机各存一张，均用组号 1。点击“下一组”后移动棋盘，按此方式拍 10～20 组。打开“双相机棋盘格外参…”，填入两套图片目录和 `intrinsics.json`，输入实测方格边长，点击计算。结果在相机 1 图片目录中的 `stereo_时间戳/extrinsics.json`；至少需要 5 组完整 9×6 棋盘配对。两台相机可使用不同分辨率，但各自照片须与各自内参匹配。可以开两个上位机窗口分别连接，或在棋盘固定时切换相机。
